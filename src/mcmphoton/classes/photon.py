...
class Photon:
    ...
    def tick(self):
        ...
        while intersection:
            ...
            distance = self.calculate_distance(
                self.position, Point3D(*intersection))
            self.MFP -= distance
            debug("MFP")
            debug(self.MFP)
    ...
    def step_length(self):
        ...
        MFP = (- np.log(1 - random.uniform(0, 1))) / \
              ((self.current_layer.scattering_coefficient +
               self.current_layer.absorption_coefficient))
    ...
    def reflect_refract(self, intersection, layer):
        ...
        # Refract
            ...
            refraction_vector = self.refraction_direction(
                self.direction, n, n_1, n_2)
    ...
    def absorb_or_scatter(self):
        ...
        """
        RND = random.uniform(0, 1)
        ratio = self.current_layer.absorption_coefficient / \
            (self.current_layer.scattering_coefficient +
             self.current_layer.absorption_coefficient)
    ...
    def scatter(self):
        ...
        if muzs == 1.0:
            ...
            muzcosfi = muzs * cosfi
            muxd = sintheta * (muxs * muzcosfi - muys *
                               sinfi) / denom + muxs * costheta
            muyd = sintheta * (muys * muzcosfi + muxs *
                               sinfi) / denom + muys * costheta
    ...
    def calculate_distance(self, p1, p2):
        ...
        """
        distance = ((p2.x - p1.x)**2 + (p2.y - p1.y)
                    ** 2 + (p2.z - p1.z)**2) ** 0.5
        return distance
        ...
        if (n_1 >= n_2) and (angle_of_incidence > math.asin(n_2/n_1)):
            ...
            try:
                refraction_angle = math.asin(
                    n_1 * math.sin(angle_of_incidence) / n_2)

                rp = -(math.sin(angle_of_incidence - refraction_angle) /
                       (math.sin(angle_of_incidence + refraction_angle)))
                rs = -(math.tan(angle_of_incidence - refraction_angle) /
                       (math.tan(angle_of_incidence + refraction_angle)))

                tp = -(2 * math.cos(angle_of_incidence) * math.sin(refraction_angle)
                       )/(math.sin(angle_of_incidence + refraction_angle))
                ts = (2 * math.cos(angle_of_incidence) * math.sin(refraction_angle))/(math.sin(
                    angle_of_incidence + refraction_angle) * math.cos(angle_of_incidence - refraction_angle))
    ...
    def recalculate_MFP(self, next_layer):
        ...
        prev_MFP_scale = (self.current_layer.scattering_coefficient +
                          self.current_layer.absorption_coefficient)
        next_MFP_scale = (next_layer.scattering_coefficient +
                          next_layer.absorption_coefficient)
