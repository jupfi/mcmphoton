    ...
    def plot_photon_paths(self, heatmap = False, debug = False):
        ...
        for i, layer in enumerate(layers):
            ...
            if i == (len(layers) - 1) or i == 0:
                continue
            ax.axhspan(layer.z_range[0], layer.z_range[1], alpha=0.15, color = colors[i - 1 % len(colors)])
        ...
        for i, source in enumerate(sources):
            ...
            for photon in photons:
                ...
                # Mark points when scattering or boundary crossing occurs for debugging 
                    ...
                    ax.plot(r, z, color = colors[-i % len(colors)], linewidth=0.1)
                    ax.plot(r, z, 'o', markersize=0.75, color = colors[-i % len(colors)])
